...
def load_and_sample_image(image_path, num_samples=10000, device="cuda"):
    ...
    if num_samples >= total_pixels:
        ...
        print(
            f"Sampled {len(sampled_pixels):,} pixels ({num_random:,} random + {len(edge_samples):,} edge)"
        )
...
def plot_rgb_space(
    ...
    ax.scatter(
        ...
        alpha=alpha,
        rasterized=True,
    )  # Rasterize for faster rendering
...
def plot_material_cylinder(
    img, material_mask, title="BIDR Cylinder for Material", save_path=None
):
    """
...
def visualize_isd_components(isd_map, save_path="isd_components.png"):
    ...
    labels = [
        "ISD_R (log-Red component)",
        "ISD_G (log-Green component)",
        "ISD_B (log-Blue component)",
    ]
...
def analyze_isd_statistics(isd_map):
    ...
    print(
        f"  Normalized: [{avg_isd_normalized[0]:.4f}, {avg_isd_normalized[1]:.4f}, {avg_isd_normalized[2]:.4f}]"
    )
...
def visualize_isd_output(predicted_isds, original_img, output_prefix="output"):
    ...
    visualize_isd_as_direction(
        predicted_isds, save_path=f"{output_prefix}_direction.png"
    )

    # 3. Component visualization
    visualize_isd_components(
        predicted_isds, save_path=f"{output_prefix}_components.png"
    )

    # 4. Magnitude check (debugging)
    visualize_isd_magnitude(predicted_isds, save_path=f"{output_prefix}_magnitude.png")

    # 5. Pseudo-RGB (quick visual check)
    visualize_isd_as_rgb_shifted(
        predicted_isds, save_path=f"{output_prefix}_pseudorgb.png"
    )

    # 6. Arrow field overlay (shows local variation)
    visualize_isd_arrow_field(
        predicted_isds, original_img, stride=40, save_path=f"{output_prefix}_arrows.png"
    )
